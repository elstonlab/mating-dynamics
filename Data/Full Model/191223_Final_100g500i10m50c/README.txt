...
Filename: 191223_FinalModel_100g500i10m50c
Directory: /pine/scr/a/e/aeallen/Longleaf/Publication/191223_FinalModel_100g500i10m50c
...
def scorefxn1(arr_parameters, time):
    ...
    expX = simulate_single_experiment1(mut_params, time, 90,SS)
    # get index of time points closest
    idx_closest_time_points = []
    ...
    #SCORE IT! using MSE
    expX_mse = (np.abs(data_mutpulse[2][~np.isnan(data_mutpulse[2])] - expX_scorefxn_data[:,0]/norm_max)).mean()
    ...
    # get index of time points closest
    idx_closest_time_points = []
    for each_time in times_mutpulse[3][~np.isnan(times_mutpulse[3])]:
        closest_idx = np.abs(time - each_time).argmin()
        idx_closest_time_points.append(closest_idx)
    ...
    #SCORE IT! using MSE
    expX_mse = (np.abs(data_mutpulse[3][~np.isnan(data_mutpulse[3])] - expX_scorefxn_data[:,0]/norm_max)).mean()
